        ...
        def  Surfacerunoff(self,ii,rainydays,coefficients):
          ...
          try:
                ...
                if  self.gotError==False:
                    ...
                    # Ch for open water and impervious area is 1 then we have
                    cal("Ch"+i+"=((1-imp_ow_area)*Ch"+i+")+imp_ow_area")                                       
                    #We only apply the rainfall effect if it is greater than the threshold
                    cal("Csr"+i+"=IF("+coefficients[7]+"<Pdaily" + i + ",(Cwp*Pdaily" + i + ")/((Cwp*Pdaily" + i + ")-("+coefficients[7]+"*Cwp)+"+coefficients[7]+"),Cwp)")
